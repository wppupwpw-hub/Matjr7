...
// Path to the responses JSON file generated by the Python script
// NOTE: Make sure this file is available in the correct directory when you deploy.
const RESPONSES_FILE = path.resolve(__dirname, '..', '..', 'responses_arrays_simple.json');
let responsesData = {};
...
async function loadResponses() {
    try {
        if (!fs.existsSync(RESPONSES_FILE)) {
            console.error("❌ responses_arrays_simple.json file not found. Please ensure it's in the correct directory.");
            return;
        }
        ...
        // Transform the nested object into a flat key-value object for faster lookup
        // This handles the format from the Python script's simplified output
        for (const arrayName in data) {
            if (data.hasOwnProperty(arrayName)) {
                const arrayData = data[arrayName];
                for (const question in arrayData) {
                    if (arrayData.hasOwnProperty(question)) {
                        responsesData[question.toLowerCase().trim()] = arrayData[question];
                    }
                }
            }
        }
